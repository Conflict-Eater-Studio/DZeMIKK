...
void dzemikk::StaticMesh::uploadToGPU() {
    ...
    glGenBuffers(1, &_vbo);
    glBindBuffer(GL_ARRAY_BUFFER, _vbo);
    glBufferData(GL_ARRAY_BUFFER, static_cast<GLsizeiptr>(_vertices.size() * sizeof(StaticVertex)),
                 _vertices.data(), GL_STATIC_DRAW);

    if (_useIndices) {
        setupIndexBuffer(_indices.data(), _indexCount);
    }
