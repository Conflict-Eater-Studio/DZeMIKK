#ifndef DZEMIKK_UICHECKBOXACTIONREGISTRY_H
#define DZEMIKK_UICHECKBOXACTIONREGISTRY_H
...
class UICheckboxActionRegistry {
  public:
    using ActionFactory = std::function<std::function<void()>(UICheckbox&)>;

    static UICheckboxActionRegistry& get() {
        static UICheckboxActionRegistry kRegistry;
        return kRegistry;
    }
    ...
    [[nodiscard]] std::function<void()> bind(const std::string& actionId,
                                             UICheckbox& button) const {
  ...
  private:
    UICheckboxActionRegistry() = default;
